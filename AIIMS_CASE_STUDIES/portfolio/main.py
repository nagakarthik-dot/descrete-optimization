...
def solve_problem():
    ...
    constraints.add_constraints(variables['new_stocks_invest'])
    
    objective = Objective(model, data)
    objective.set_objective(variables['new_stocks_invest'])

    final_table = FinalTable(model,data, variables)
    table_output = final_table.print_table()
    
    print(table_output)
    final_table.save_output('stocks.txt', table_output)
