...
def solve_problem():
    ...
    constraints.add_constraints(variables['x'],variables['y'],variables['v'])
    
    objective = Objective(model, data)
    objective.set_objective(variables['x'],variables['y'],variables['v'])

    final_table = FinalTable(model,data, variables)
    table_output = final_table.print_table()
    
    print(table_output)
    final_table.save_output('path.txt', table_output)
