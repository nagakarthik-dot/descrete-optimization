    ...
    for i in range(data.num_dishes):
        for h in range(data.num_hours):
            if h >= data.shelf_life[i] and h<data.num_hours-1:
                ...
                model.addConstr(waste[i, h]+waste[i,h-1] >=  sum(prepare[i,t]-sold[i,t] for t in range(h - data.shelf_life[i]+1))  - sum(sold[i,h] for i in range(h - data.shelf_life[i]+1,h+1)))
